...
dtdnssync_config parse_config(const std::string &cfg_path) {
  // check file permissions
  {
    ...
    if (::stat(cfg_path.c_str(), &file_stat) < 0) {
      throw std::runtime_error{"Can not stat configuration file " + cfg_path +
                               ": " + ::strerror(errno)};
    }
  ...
  while (std::getline(cfg_file, line)) {
    ...
    if (key == "interval") {
      ...
      if (interval == ULONG_MAX or interval == 0 or interval == ULLONG_MAX) {
        throw std::runtime_error{"Parse error, invalid interval value: " +
                                 value};
      } else if (interval > 10080) {  // more than a week
        throw std::runtime_error{
            "Parse error, interval value to big (> 10080): " + value};
      }
