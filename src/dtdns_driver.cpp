...
asio::ip::address task_externip(asio::io_service & io_service, asio::ssl::context & ssl_ctx)
{
    ...
    return asio::ip::address::from_string(std::string(trim(std::string{ response, pos2 + 4, response.length() - (pos2 + 4) })));
}

void task_updateip(asio::io_service & io_service, const std::string & hostname, const std::string & password, asio::ssl::context & ssl_ctx)
{
    asio::ssl::stream<asio::ip::tcp::socket> socket{ io_service, ssl_ctx };
    asio::ip::tcp::resolver resolver{ io_service };
    ...
    const std::string response_content = trim(response.substr(pos2 + 4, response.length() - (pos2 + 4)));
